 ...
 * zhanxp@me.com
 */
function MysqlDao(mysql, database, table) {
    ...
    this.insertBatch = async function (datas) {
        return await db.insert(this.table, datas);
    }
    ...
    this.updateBatchById = async function (datas) {    // Update multiple rows with primary key: id
        return await db.updateRows(this.table, datas);
    };
    this.updateBatch = async function (options) {    // Update multiple rows with row and where properties
        return await db.updateRows(this.table, options);
    };

    this.get = async function (where) {
        return await db.get(this.table, where);
    };
    ...
    this.list = async function (where, columns, orders, limit, offset = 0) {
        ...
        if (options == {}) {
            return await db.select(this.table);
        } else {
            return await db.select(this.table, options);
        }
    };
