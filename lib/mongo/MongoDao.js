...
function MongoDao(app, table) {
    ...
    this.update = async function (query, data) {
        return await this.app.mongo.db.collection(this.table).updateOne(query, { $set: data });
    };

    this.updateById = async function (id, data) {
        return await this.app.mongo.db.collection(this.table).updateOne({ _id: new ObjectID(id) }, { $set: data });
    };

    this.delete = async function (query) {
        return await this.app.mongo.db.collection(this.table).deleteMany(query);
    };
